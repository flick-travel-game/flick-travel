#!/usr/bin/env python3
"""tools/spots-japan.tsv / spots-world.tsv の名所を index.html に書きこむ。
...
def read_tsv(path):
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        rows.append(dict(key=key, n=name, c=place, r=yomi, q=query, d=desc, lat=float(lat), lon=float(lon)))
    return rows
...
def main():
    ...
    spots = ",\n".join(f'  {{n:"{r["n"]}", c:"{r["c"]}", r:"{r["r"]}", art:"{r["key"]}", d:"{r["d"]}"}}' for r in rows)
    ...
    print(f"{len(rows)}か所を書いた(日本 {sum(r['c'].startswith('日本') for r in rows)} / 世界 {sum(not r['c'].startswith('日本') for r in rows)})")
