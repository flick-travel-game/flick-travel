...
- **世界一周マップ**: クリアしたレベルの名所に ピンが立つ。**やった所だけ 📍。まだの所は 出さない**(2026-09-26 けいくん「赤ピンだと地図が赤くなって 見えなくなってくる」→「やってない問題の白ピンはすべて削除して」→「📍にする?」)。
  **地図は 2本の指(ピンチ)・＋－・ダブルタップ・Ctrl+ホイールで 8倍まで 大きくできる**(同日「もっと地図を拡大出来るようにならないと 目的のピンにたどり着かない」)。
  `.map-in` を transform で 大きくし、📍と 海の名前は `--k`(=1/倍率)で 小さくもどすので **📍の大きさは 変わらず 場所だけ 離れる**(ブラウザの拡大では 📍も 大きくなって 重なったまま だった)。しくみは `index.html` の `MZ` / `mapApply` / `mapZoomAt`。地図を切りかえると もどる。
  **1.2倍をこえたら 4320px の地図(`world-map-color-4k.jpg` / `japan-map-color-4k.jpg`。`MAPS.*.hi`)に 差しかえる**(同日「拡大すると地図がボヤけて見えない」)。はじめは 1080px。4k は scratchpad の `render.mjs` で 同じ SVG(`world-color.svg` / `japan-color.svg`)を 4320 幅で描いて JPEG q80 にしたもの地図は **`world-map-color.jpg`**(2026-09-24 けいくん「世界フリック旅行みたいな カラフルで綺麗な画像にしたい」→
