...
- **だれが あそぶか(6人まで)**(2026-09-25 けいくん「かずとものように右上にアイコンを作って 6人まで登録出来るように」): 右上の 👤 ボタン → 名前(8文字まで)・学年・アイコン。
  端末の中だけ(`flick-profiles` / `flick-profile`)。**自己ベストとピンは 人ごと**(`flick-best-p<id>-<旅>-<レベル>`。`bestKey()` の1か所)。
  だれも登録していないときは いままでどおり `flick-best-<旅>-<レベル>`。**はじめて登録した人に それまでの記録を引きつぐ**。名前・学年は どこにも送らない
...
- **みんなのランキング(学年ごと)**(2026-09-25 けいくん「スピードランキングも かずとものように 学年順で競走出来るように」→「1」):
  ...
  ここには 鍵も 表も 無い(`RANK_API` の 住所だけ)。10問 打ち終わると、👤 で 登録した人なら 記録を送り(`submitScore`)、結果画面の
  「🏆 みんなのランキング」に 同じ学年の 速い順 20人(きょう / 今週 / いままで)。登録していない人には「登録すると くらべられるよ」。
